 ...
 * <br> Keyspace : # = ?
 * <br> Encryption :
 * <p>In the rail fence cipher, the plaintext is written downwards diagonally on successive "rails" of an imaginary
 * fence, then moving up when the bottom rail is reached, down again when the top rail is reached, and so on until the
 * whole plaintext is written out. The ciphertext is then read off in rows.</p>
 * <br> Decryption :
 * <p>Let N be the number of rails used during encryption.Observe that as the plaintext is written,
 * the sequence of each letter's vertical position on the rails varies up and down in a repeating cycle. In the above
 * example N=3 the vertical position repeats with a period of 4. In general the sequence repeats with a period of
 * 2(N-1). Let L be the length of the string to be decrypted. Suppose for a moment that L is a multiple of 2(N-1) and
 * let K=L/2(N-1).One begins splitting the ciphertext into strings such that the length of the first and last string is
 * K and the length of each intermediate string is 2K For above example L=24 and we have K=6, so se split text into 6 12
 * 6</p>
 * <br>
 * <br>
 * For more read : <br>
 * <a href="https://en.wikipedia.org/wiki/Rail_fence_cipher">Rail fence link</a>
 * <a href="https://www.baeldung.com/cs/cryptography-rail-fence-cipher">cryptography-rail-fence-cipher</a>
 */
    ...
    if (params instanceof ParameterList) {
      ...
      for (CipherParameters param : parameterList) {
        if (param instanceof KeyParameter) {
          this.rails = PrimitiveUtils.getInt(((KeyParameter) param).getKey());
          this.encryption = encryption;
          return;
        } else if (param instanceof WrappedKeyParameter) {
          this.rails = ((WrappedKeyParameter<RailfenceKey>) param).getWrappedKey().getRails();
          this.encryption = encryption;
          return;
        }
      }
      throw new IllegalArgumentException("Invalid parameter passed to Railfence init - key parameter not found.");
    } else if (params instanceof WrappedKeyParameter) {
      this.rails = ((WrappedKeyParameter<RailfenceKey>) params).getWrappedKey().getRails();
      this.encryption = encryption;
    } else if (params instanceof KeyWithIVParameter) {
      KeyWithIVParameter keyIvParam = (KeyWithIVParameter) params;
      this.rails = PrimitiveUtils.getInt(keyIvParam.getKey());
      this.encryption = encryption;
    } else if (params instanceof KeyParameter) {
      KeyParameter keyParameter = (KeyParameter) params;
      this.rails = PrimitiveUtils.getInt(keyParameter.getKey());
      this.encryption = encryption;
    } else {
      throw new IllegalArgumentException("Invalid parameter passed to Railfence init - " + params.getClass().getName());
    }
  }
