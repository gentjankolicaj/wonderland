...
import io.wonderland.alice.crypto.cipher.key.MonoalphabetKey;
 ...
 * <br>
 * <br>
 * For more check <a href="https://en.wikipedia.org/wiki/Substitution_cipher">Monoalphabet cipher link</a>
 */
@Slf4j
public final class MonoalphabetCrypt implements StreamCipher {
    ...
    if (params instanceof ParameterList) {
      ...
      for (CipherParameters param : parameterList) {
        if (param instanceof WrappedKeyParameter) {
          this.key = ((WrappedKeyParameter<MonoalphabetKey>) param).getWrappedKey().getKey();
          this.encryption = encryption;
          return;
        }
      }
      throw new IllegalArgumentException("Invalid parameter passed to Monoalphabet init - key parameter not found.");
    } else if (params instanceof WrappedKeyParameter) {
      this.key = ((WrappedKeyParameter<MonoalphabetKey>) params).getWrappedKey().getKey();
      this.encryption = encryption;
    } else {
  ...
  public byte processByte(byte in) {
    ...
    if (encryption) {
      return (byte) (int) key.get((int) in);
    } else {
      for (Map.Entry<Integer, Integer> entry : key.entrySet()) {
