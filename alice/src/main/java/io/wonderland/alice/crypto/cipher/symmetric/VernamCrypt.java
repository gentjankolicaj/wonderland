...
public final class VernamCrypt implements StreamCipher {
  ...
  public int encrypt(byte[] in, int inOff, byte[] out, int outOff) throws CipherException {
    ...
    if (key.length < in.length) {
      throw new IllegalArgumentException("Vernam cipher requires key length bigger/equal to plaintext length.");
    }
  ...
  public int decrypt(byte[] in, int inOff, byte[] out, int outOff) throws CipherException {
    ...
    if (key.length < in.length) {
      throw new IllegalArgumentException("Vernam cipher requires key length bigger/equal to ciphertext length.");
    }
    int len = in.length - inOff;
    for (int i = 0; i < len; i++) {
      byte c = in[inOff + i];
  ...
  public void init(boolean encryption, CipherParameters params) throws IllegalArgumentException {
    if (params instanceof ParameterList) {
      ...
      throw new IllegalArgumentException("Invalid parameter passed to Vernam init - key parameter not found.");
      ...
      throw new IllegalArgumentException("Invalid parameter passed to Vernam init - " + params.getClass().getName());
    }
  }

  @Override
  public String getAlgorithmName() {
    return AlgNames.Vernam;
  }
