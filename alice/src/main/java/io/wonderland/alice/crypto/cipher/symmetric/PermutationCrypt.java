...
import io.wonderland.alice.crypto.cipher.key.PermutationKey;
...
public class PermutationCrypt implements TranspositionCipher, BlockCipher {
  ...
  public void transpose(byte[] in, byte[] out) throws CipherException {
    ...
    if (ArrayUtils.isEmpty(columnOrders)) {
      throw new IllegalArgumentException(ExceptionMessages.KEY_NOT_VALID + " Column order not valid.");
    }
    if (encryption) {
      encrypt(in, out);
    } else {
  ...
  private void encrypt(byte[] in, byte[] out) {
    ...
    int rowNum =
        in.length % columnOrders.length == 0 ? in.length / columnOrders.length : (in.length / columnOrders.length) + 1;
  ...
  boolean canPad(int[] columnOrder, byte[] in) {
    ...
    int rowNum =
        in.length % columnOrder.length == 0 ? in.length / columnOrder.length : (in.length / columnOrder.length) + 1;
    ...
    if (params instanceof ParameterList) {
      ...
      for (CipherParameters param : parameterList) {
        if (param instanceof WrappedKeyParameter) {
          PermutationKey permutationKey = ((WrappedKeyParameter<PermutationKey>) param).getWrappedKey();
          this.columnOrders = permutationKey.getColumnOrder();
          this.encryption = encryption;
          return;
        }
      }
      throw new IllegalArgumentException("Invalid parameter passed to Permutation init - key parameter not found.");
    } else if (params instanceof WrappedKeyParameter) {
      PermutationKey permutationKey = ((WrappedKeyParameter<PermutationKey>) params).getWrappedKey();
      this.columnOrders = permutationKey.getColumnOrder();
      this.encryption = encryption;
    } else {
