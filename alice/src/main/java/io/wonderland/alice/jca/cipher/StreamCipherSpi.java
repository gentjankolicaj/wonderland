...
public abstract class StreamCipherSpi extends CipherSpi {
  ...
  private KeyWithIVParameter keyWithIVParameter;
  ...
  protected StreamCipherSpi(StreamCipher cipher, int keySizeInBits, KeyWithIVParameter keyWithIVParameter) {
  ...
  /**
   * Cipher block padding supported for cipher streams are: NoPadding because streams don't need padding.
   *
  ...
  protected void engineInit(int opmode, Key key, AlgorithmParameterSpec algorithmParameterSpec,
    ...
    ParameterList parameterList = new ParameterList(new WrappedKeyParameter<>(key));
  ...
  protected byte[] engineDoFinal(byte[] input, int inputOffset, int inputLen) {
    if (inputLen != 0) {
      byte[] output = this.engineUpdate(input, inputOffset, inputLen);
      this.cipher.reset();
      return output;
    } else {
      this.cipher.reset();
