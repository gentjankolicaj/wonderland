...
public class DataMatrixCode extends Barcode {

  private static final DataMatrixWriter BARCODE_WRITER = new DataMatrixWriter();
  private static final double CODE_TO_VIEW_RATIO = 0.6;
  private static final double DEFAULT_HEIGHT = 700;

  public DataMatrixCode(CodecAlg<byte[], String, String, byte[]> codecAlg) {
    super(codecAlg);
  }

  public static BufferedImage generateQRCodeImage(String barcode, int width, int height) {
    BitMatrix bitMatrix = BARCODE_WRITER.encode(barcode, BarcodeFormat.DATA_MATRIX, width, height);
