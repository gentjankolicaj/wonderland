...
public final class IndexCoincidence {
  ...
  public static Map<Integer, Double> calcKeyPeriodIC(byte[] ciphertext, int encodingBytes, int minKeyLength,
      int maxKeyLength) {
    if (maxKeyLength > (ciphertext.length / encodingBytes)) {
      throw new IllegalArgumentException("Max key length search can't be bigger than ciphertext/encodingBytes");
    }
  ...
  public static List<Byte>[] createCiphertextMat(byte[] ciphertext, int encodingBytes, int keyLength) {
