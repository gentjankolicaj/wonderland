...
public class MPPAlgorithm {
  ...
  public static Map<Monochar, List<MPPValue<? extends Char>>> monocharCalc(Map<Monochar, Double> langFreq,
    ...
    for (Map.Entry<Monochar, Double> langEntry : langFreq.entrySet()) {
      ...
      List<MPPValue<? extends Char>> sortedList = list.stream().sorted(Comparator.comparing(MPPValue::getFreqDiff))
          .collect(
              Collectors.toList());
