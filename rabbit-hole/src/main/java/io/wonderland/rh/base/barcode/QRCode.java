...
public class QRCode extends Barcode {
  ...
  private static final QRCodeWriter BARCODE_WRITER = new QRCodeWriter();
  private static final double CODE_TO_VIEW_RATIO = 0.6;
  private static final double DEFAULT_HEIGHT = 700;

  public QRCode(CodecAlg<byte[], String, String, byte[]> codecAlg) {
    super(codecAlg);
  }

  public static BufferedImage generateQRCodeImage(String barcode, int width, int height)
      throws WriterException {
    BitMatrix bitMatrix = BARCODE_WRITER.encode(barcode, BarcodeFormat.QR_CODE, width, height);
    return MatrixToImageWriter.toBufferedImage(bitMatrix);
  }

  public static ImageView getImageView(Pane pane, String encoded) throws WriterException {
